...
class pqDB:
       PQ_SEM = None;
        ...
        def __init__(self, database, host):
                ...
                if len(host) > 0:
                        self.DBHOST=host
                self.con = psycopg2.connect("dbname=" + self.DB + self.DBHOST + " password=jmp81tmr")
                self.PQ_SEM = threading.BoundedSemaphore(1)
        #
        def __init__(self, database, host, uid, passwd):
                if database is not None and len(database) > 0:
                        self.DB=database
                ...
                if passwd is not None and len(passwd) > 0:
                        self.con = psycopg2.connect("dbname=" + self.DB + " user=" + self.UID + " host=" + self.DBHOST + " password=" + passwd)
                else:
                        self.con = psycopg2.connect("dbname=" + self.DB + " user=" + self.UID + " host=" + self.DBHOST)
                self.PQ_SEM = threading.BoundedSemaphore(1)
        #
	        def close_db(self):
                print "close_db()"
